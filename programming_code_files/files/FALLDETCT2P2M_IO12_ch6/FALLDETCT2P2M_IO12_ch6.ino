...
void setup() {
  ...
  // Optional Radar setup (commented if you only use IO pins)
  ...
  hu.configLEDLight(hu.eFALLLed, 1); // FALL LED ON
  hu.configLEDLight(hu.eHPLed, 1);   // PRESENCE LED ON

  // Fall detection parameters
  hu.dmInstallHeight(220);              // 220 cm mounting height
  hu.dmFallTime(2);                     // 2 sec fall confirmation
  hu.dmUnmannedTime(1);                 // 1 sec empty detection
  hu.dmFallConfig(hu.eResidenceTime, 10);// 10 sec dwell detection
...
void handleRoot() {
  ...
  html += "<h2>Movement Range (UART):</h2>";
